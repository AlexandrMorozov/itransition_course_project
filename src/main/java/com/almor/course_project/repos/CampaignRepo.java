...
public interface CampaignRepo extends CrudRepository<Campaign, Long> {
    ...
    @Query("SELECT c FROM Campaign c ORDER BY c.lastUpdateDate DESC")
    List<Campaign> findRecentlyUpdatedCampaigns(Pageable pageable);

    @Query("SELECT new com.almor.course_project.dto.CampaignRatingDto(c.id, c.name, c.description, AVG(ur.rating))" +
            " FROM Campaign c " +
